...
const getKeyPair = key => (key ? parseKeyPair(key) : rsa.generateKeyPair(2046))
...
// #define GEN_RID         8
const buildSANExtension = domain =>
  isIPv4(domain)
    ? {
      name: 'subjectAltName',
...
function createCertificate(domain, extensions = []) {
  ...
  cert.setSubject(defaultAttrs.concat([{ name: 'commonName', value: domain }]))
