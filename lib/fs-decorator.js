...
const readDomainKey = async domain =>
  fs.readFile(strFormat(config.keyPath, domain), 'ascii')

const readDomainCert = async domain =>
  fs.readFile(strFormat(config.crtPath, domain), 'ascii')

const writeDomainKey = async (domain, pem) =>
  fs.writeFile(strFormat(config.keyPath, domain), pem)

const writeDomainCert = async (domain, pem) =>
  fs.writeFile(strFormat(config.crtPath, domain), pem)

const exists = async domain => fs.exists(strFormat(config.crtPath, domain))
...
module.exports = async (domain, target) =>
  (await load(domain)) || save(domain, await target(domain))
